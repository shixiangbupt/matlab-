...
while feof(fid) == 0
    aline=fgetl(fid);
    aline=aline(2:end);
    [xstr rest]=strtok(aline,'y');%函数用于自左向右提取字符串中某字符或字符串，提取的依据是给定的delimiter
    x=[x str2num(xstr)];
    ystr=rest(2:end);
    y=[y str2num(ystr)];
end
...
% * 第二种方法
% 用fsanf
% 需要指定文件的每行的格式：字符，数字，字符，数字，换行符
% 创建的矩阵会将文件的每行存储为单独的列，维数是4*n，n是文件行数(若不确定inf）
% x字符在矩阵的第一行(‘x’为Ascii码），x坐标值在第二行，‘y’等价的ascii值在第三列，第四行存储y坐标值。
mat=fscanf(fid,'%c%f%c%f\n',[4 inf]);%换行符必须有
x=mat(2,:);y=mat(4,:);
%%
% 
% * 第三种方法：还是用fscanf
% 因为每行中都包含'x','y'，可以将其创建为格式化字符串
mat = fscan(fid,'x%fy%f\n',[2,inf]);
x=mat(1,:);
y=mat(2,:);
%这种情况，字符'x','y'不会被读入矩阵
%%
...
%不需要换行符的原因是，使用textscan时，格式化字符串指定了文件中的列的格式，而对fscanf，指定了文件每行的格式
x=xydat{1};
y=xydat{2};

%%
% 
% * 总结：四种读取文件的方法，load函数只适用于文件中类型相同且每行值的个数相同，这样可以把它们读入一个矩阵中。
...
% c 432 bangongshi
% d 312 Xinke
 [nums,txt]=xlsread('texttest.xls');%会把数值读到一个double型向量中，文本读到一个元胞数组txt中
 txt{1,2}
 txt{1,3}
 %通过循环再次打印表格中的值
 for i=1:length(nums)
